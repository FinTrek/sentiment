classify_polarity <- function(textColumns, 
	...
	lexicon_filename <- paste("data/subjectivity_", language, ".csv.gz", 
	                          sep = "")
	lexicon <- read.csv(system.file(lexicon_filename, 
	                                package = "sentiment"), 
	                    header = FALSE)
	...
	for (i in 1:nrow(matrix)) {
		...
		scores <- list(positive = 0, 
		               negative = 0)
		doc <- matrix[i, ]
		words <- findFreqTerms(doc, 
		                       lowfreq = 1)
